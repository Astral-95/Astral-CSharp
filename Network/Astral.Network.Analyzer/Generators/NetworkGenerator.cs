...
public class NetworkGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext Context)
    {
        var ConfigProvider =
            Context.AdditionalTextsProvider
                .Where(File => Path.GetFileName(File.Path) == "AstralNetworkGenConfig.txt")
                ...
                .Select((Files, _) =>
                {
                    ...
                    if (Files.Length > 1)
                        throw new InvalidOperationException(
                            "Multiple AstralNetworkGenConfig.txt files found");
    ...
    {
        if (!ClassSymbol.AllInterfaces.Contains(Core.NetworkInterface)) return;

        //ReportException(Spc, "ToDisplayString", Ex);

        NetworkClass Class = new NetworkClass(Core.Compilation, Spc, ClassSymbol, Core.NetworkInterface);

        if (Class.Code == "") return;
        var GeneratedFileName = $"{ClassSymbol.Name}_Network.gen.cs";
