...
public class PacketFragmentationHandler
{
	class FragmentedPacket
	{
		protected int InPool = 0;
        private static readonly ConcurrentStore<FragmentedPacket> Pool = new();
	...
	private object InLockObject = new();
	private object OutLockObject = new();

	NetaConnection Connection;

	Neta_FragmentGroupIndexType NextGroupIndex = 0;
	const Neta_FragmentGroupIndexType MaxGroupIndex = 4096;

	FragmentsGroup[] Groups = new FragmentsGroup[MaxGroupIndex];
	...
	internal PacketFragmentationHandler(NetaConnection Connection)
	{
		...
		{
			Groups[Index] = new FragmentsGroup();
		}
	}

	class PacketFragmentationHandler_ProcessIncomingPacket { }
	public PooledInPacket? ProcessIncomingPacket(PooledInPacket Packet)
	{
		lock (InLockObject)
		{
...
#endif
			FragmentsGroup Group = Groups[Frag.GroupIndex];
			...
			var CombinePacket = PooledInPacket.Rent(Writer);
	...
	List<OutFragmentInfo> FragmentInfos = new List<OutFragmentInfo>(256);

	class PacketFragmentationHandler_ProcessOutgoingPacket_1 { }
	class PacketFragmentationHandler_ProcessOutgoingPacket_2 { }
	public void ProcessOutgoingPacket(PooledOutPacket Packet, List<PooledOutPacket> OutPackets, Int64 TicksNow)
	{
#if NETA_DEBUG
